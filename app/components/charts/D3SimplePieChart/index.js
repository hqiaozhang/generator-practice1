...
export default class Pie {
   ...
   */
  render(data) {
    console.log('chart data*********', data, !data || !data.length)
    const self = this
  ...
  setTextAttribute(text, type) {
    ...
    text.attr('fill', (d, i) => `${type === 1 ? '#fff' : self.getColor(i)}`)
      ...
      .attr('opacity', 1)
      .attr('transform', (d) => {
        let pos = self.lEndArc.centroid(d) // centroid(d)计算弧中心   
        let radius = outerRadius + 60 // 位置偏移
        // 判断当前圆弧的位置, 将文字移动到外面去
